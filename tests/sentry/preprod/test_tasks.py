...
class AssemblePreprodArtifactTest(BaseAssembleTest):
    ...
    @patch("sentry.preprod.tasks._dispatch_taskbroker_shadow")
    @patch("sentry.preprod.tasks.produce_preprod_artifact_to_kafka")
    def test_shadow_taskbroker_failure_does_not_affect_kafka(
        self, mock_produce_to_kafka, mock_shadow
    ) -> None:
        mock_shadow.side_effect = Exception("taskbroker failure")

        content = b"test shadow taskbroker failure isolation"
        fileobj = ContentFile(content)
        total_checksum = sha1(content).hexdigest()
